...
@ModelAttribute
    ...
    public String profile(ModelMap model,HttpSession session){
        Librarian lib= (Librarian) session.getAttribute("value");
        System.out.println(lib.getFullName()+" "+lib.getEmail());

         LibrarianDto librarianDto=new LibrarianDto();
         librarianDto.setFullName(lib.getFullName());
         librarianDto.setEmail(lib.getEmail());
         librarianDto.setAddress(lib.getAddress());
         librarianDto.setContact(lib.getContact());
         librarianDto.setPassword(lib.getPassword());
         model.addAttribute("librarianDto",librarianDto);
         model.addAttribute("lib",lib);
         return "profile";
    }
    @PostMapping("/profile/{id}")
    public String setProfile(@Valid @ModelAttribute ("librarianDto") LibrarianDto librarianDto, BindingResult result, @PathVariable("id") int id, ModelMap model, HttpSession session){
           try{
               model.addAttribute("librarianDto",librarianDto);
               Librarian lib1=new Librarian();
               lib1.setId(id);
               lib1.setFullName(librarianDto.getFullName());
               lib1.setEmail(librarianDto.getEmail());
               lib1.setAddress(librarianDto.getAddress());
               lib1.setContact(librarianDto.getContact());
               lib1.setPassword(librarianDto.getPassword());
               libRepo.save(lib1);
               session.setAttribute("msg14","Profile update Successfully!!");
               return "redirect:/login";
           }
           catch(Exception e){
               System.out.println(e.getMessage());
               session.setAttribute("msg14","Something went wrong");
               return "profile";
           }
    }
    // User Controller
...
public String listUser(ModelMap model,HttpSession session,@RequestParam(defaultValue = "") String keyword){
